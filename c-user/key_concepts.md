...
The $O(m)$ Independence-Preserving Protocol (OMIP) is a generalization of
...
{cite}`Brandenburg:2013:OMIP`. The $m$ denotes the number of processors
...
$O(1)$ worst-case time complexity for enqueue and dequeue operations.

A red-black tree is used to implement the priority queues yielding a
$O(log(n))$ worst-case time complexity for enqueue and dequeue operations
with $n$ being the count of threads already on the queue.
...
$O(log(n))$ worst-case insert and removal operations for $n$ active
...
based algorithm offers $O(1)$ worst-case time complexity for insert and
