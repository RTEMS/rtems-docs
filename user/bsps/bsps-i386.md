...
```{note}
BSP capability to detect target hardware SMP details is
...
1. prepare RTEMS binary and save it either to Linux
   partition/directory accessible from GRUB or to an USB stick.
2. boot machine to GRUB menu.

```{note}
Some Linux installations hide GRUB menu by default and
...
4. use `ls` command to observe drives and partitions on them. If
...
```{note}
Use `ls (hdX, partY)` without a slash at the end to show
information about the partition.
```

22. use `multiboot` command to load the RTEMS application binary for
...
```{note}
Advantage of using GRUB for booting RTEMS is the GRUB's
...
```{note}
It is not possible to use UEFI based PXE booting. Neither
