...
```{note}
The list is growing all the time and if your BSP is not supported we
...
```{note}
Some BSPs may require a post-build process to be run on the RTEMS ELF
...
```{note}
If the path to the testsuites was set to
...
```{note}
Some BSPs require the use of the specific target architecture GDB
