...
: `rtems_file_descriptor_type` examines some of the upper bits of the file
  ...
  3. Determine if there is actually a function in the selected handler table
  ...
  4. If the function has been defined it is invoked with the file control
     block pointer as its argument.
  5. The file control block that was associated with the open file descriptor
...
: The only commands that have been implemented are the F_GETFD and F_SETFD.
  ...
  1. Test to see if we are trying to operate on a file descriptor associated
...
: This routine validates that the struct stat pointer is not NULL so that the
  ...
  1. Obtain the file control block that is associated with the file descriptor
     index.
  2. Range check the file descriptor index.
  3. Test to see if there is a non-NULL function pointer in the handler table
...
: The file descriptor is examined to determine if it is associated with a
  ...
  1. The file descriptor index is used to obtain the associated file control
     block.
  2. The file descriptor value is range checked.
  3. A test is made to determine if the handler table that is referenced in
...
: This routine will determine if the file descriptor is associated with a
  ...
  1. The file descriptor is used to obtain the file control block for the
     node.
  2. The file descriptor is range checked.
  3. The offset element of the file control block is altered as indicated by
     the offset and whence calling parameters
  4. The handler table in the file control block is examined to determine if
     it contains an entry for the lseek() function. If it does not an error
     is returned to the calling program.
  5. The lseek() function from the designated handler table is called with
     the file control block, offset and whence as calling arguments
  6. The return code from the lseek() handler function is returned to the
     calling program
...
: This routine will examine the type of file descriptor it is sent.
  ...
  6. If there is a read function in the handler table, invoke the handler
     table read() function
  7. Use the return code from the handler table read function(number of bytes
...
: - This routine will determine if there are any filesystems currently
  ...
  1. Call the filesystem specific unmount() function for the filesystem that
     contains the mount point. This routine should indicate that the mount
     point no longer has a filesystem mounted below it.
  2. Call the filesystem specific fsunmount_me() function for the mounted
  ...
  3. Extract the mount table entry for the filesystem that was just dismounted
